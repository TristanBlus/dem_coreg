...
def destripe(dh, mask=None, filt_sz=5, std_th=0.05, percentile_th=97.5):
    ...
    while abs(STDs[iteration] - STDs[iteration - 1]) > STDs[iteration - 1] * std_th:
        ...
        if np.ma.array(dh, mask=mask).std(dtype='float64') > 20:
        ...
        # destriping using non-glac area
        if mask is not None:
            dh_foo[mask] = 0
        ...
        fft = np.fft.fftshift(np.fft.fft2(dh_foo))
        ...
        # convert to stripes
        stripes_tmp = np.real(np.fft.ifft2(np.fft.ifftshift(fft_filt)))

        dh_new = dh - stripes_tmp
        ...
        iteration += 1
        if iteration == 11:
            print("maxmimum iterations (10) reached, breaking script, something's fishy with the tile")
            break
