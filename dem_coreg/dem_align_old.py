...
def get_mask(ds, mask_list, dem_fn=None, writeout=False):
    #This returns True (1) for areas to mask, False (0) for valid static surfaces
    static_mask = dem_mask.get_mask(ds, mask_list, dem_fn, writeout=writeout)
    #return ~(static_mask)
    return static_mask
...
def get_filtered_slope(ds, slope_lim=(0.01, 50)):
    #Generate slope map
...
def compute_offset(ref_dem_ds, src_dem_ds, src_dem_fn, mode='nuth', remove_outliers=True, max_offset=100, \
        max_dz=100, slope_lim=(0.01, 50), mask_list=['glaciers',], plot=True):
...
def main(argv=None):
    ...
    while True:
        ...
        if n > max_iter or dm < tol:
            ...
            if True:
                ...
                static_mask_glac = get_mask(src_dem_clip_ds_align, mask_list, src_dem_fn, writeout=True)
                static_mask_glac = np.logical_or(np.ma.getmaskarray(diff_align), static_mask_glac)
                
                
                #Final stats, before outlier removal
                diff_align_compressed = diff_align[~static_mask_glac]
                diff_align_stats = malib.get_stats_dict(diff_align_compressed, full=True)

                #Prepare filtered version for tiltcorr fit

                #diff_align_filt_nonglac = outlier_filter(diff_align_filt_nonglac, perc=(12.5, 87.5), max_dz=max_dz)
                # static_mask_gla = np.ma.getmaskarray(diff_align_filt_nonglac)
                slope = get_filtered_slope(src_dem_clip_ds_align)
                static_mask_final = np.logical_or(static_mask_glac, np.ma.getmaskarray(slope))

                diff_align_filt_nonglac = np.ma.array(diff_align, mask=static_mask_final)
                diff_align_filt_compressed = diff_align_filt_nonglac[~static_mask_final]
                diff_align_filt_nonglac = outlier_filter(diff_align_filt_nonglac, f=3, max_dz=max_dz)
                # static_mask_gla = np.logical_or(static_mask_final, slope_mask)
                # diff_align_filt_nonglac = np.ma.array(diff_align_filt_nonglac, mask=np.ma.getmaskarray(slope))
                diff_align_filt_stats = malib.get_stats_dict(diff_align_filt_nonglac, full=True)

                # mask_glac = np.array([~static_mask_final])
                ...
                # replace the diff_align_filt_mask with static_mask_glac, if the glacier outline is corrected
                diff_align_filt[static_mask_glac == True] = diff_align[static_mask_glac == True]
            ...
            if tiltcorr and not tiltcorr_done:
                ...
                vals, resid, coeff = geolib.ma_fitpoly(diff_align_filt_nonglac, order=polyorder, gt=gt, perc=(2,98), origmask=False)
    ...
    if True:
        ...
        diff_orig_filt = outlier_filter(diff_orig_filt, f=3, max_dz=300)
    ...
    if True:
        ...
        axa[0,2].imshow(~static_mask_glac, clim=(0,1), cmap='gray')
        axa[0,2].set_title('Surfaces for co-registration')
        axa[0,2].set_facecolor('w')
        pltlib.hide_ticks(axa[0,2])

        dz_clim = malib.calcperc_sym(diff_align_filt[static_mask_glac], (2, 98))
        dz_clim_noglac = malib.calcperc_sym(diff_orig_compressed, (2, 98))
        ...
        axa[1,0].imshow(src_dem_hs, cmap='gray')
        im = axa[1,0].imshow(diff_orig, cmap='cpt_rainbow', clim=dz_clim, alpha=0.6)
        ...
        axa[1,1].imshow(src_dem_hs, cmap='gray')
        im = axa[1,1].imshow(diff_align, cmap='cpt_rainbow', clim=dz_clim, alpha=0.6)
        ...
        axa[1,2].imshow(src_dem_hs, cmap='gray')
        im = axa[1,2].imshow(diff_align_filt, cmap='cpt_rainbow', clim=dz_clim, alpha=0.6)
        ...
        tight_dz_clim = (-10, 10)
        axa[1,3].imshow(src_dem_hs, cmap='gray')
        im = axa[1,3].imshow(diff_align_filt_nonglac, cmap='cpt_rainbow', clim=tight_dz_clim, alpha=0.6)
