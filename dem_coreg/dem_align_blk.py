...
def get_mask(ds, mask_list, dem_fn=None, writeout=False, erode=False):
    ...
    static_mask = dem_mask.get_mask(ds, mask_list, dem_fn, writeout=writeout, erode=erode)
    # return ~(static_mask)
    return static_mask
...
def outlier_filter(diff, f=3, perc=None, max_dz=100):
    ...
    # Absolute dz filter
    diff = np.ma.masked_greater(diff, abs(max_dz))
    diff = np.ma.masked_less(diff, -abs(max_dz))
    print(diff.count())
    ...
    if perc is not None:
        ...
        print("Only absolute dz filter used.")

    print(diff.count())
    return diff


def get_filtered_slope(ds, slope_lim=(0.01, 45)):
    # Generate slope map
...
def compute_offset(ref_dem_ds, src_dem_ds, src_dem_fn, mode='nuth', remove_outliers=True, max_offset=100,
                   max_dz=100, blk_sz=(300,300), slope_lim=(0.01, 50), mask_list=['glaciers', ], plot=True):
    ...
    ref_dem_clip_ds, src_dem_clip_ds = warplib.memwarp_multi([ref_dem_ds, src_dem_ds],
                                                             res='max', extent='intersection', t_srs=src_dem_ds,
                                                             r='cubic')

    # Compute size of NCC and SAD search window in pixels
    res = geolib.get_res(ref_dem_clip_ds, square=False)
    if len(res) > 1:
        max_offset_px = [((max_offset / res[0]) + 1), ((max_offset / res[1]) + 1)]
    else:
        max_offset_px = [((max_offset / res) + 1), ((max_offset / res) + 1)]
    # print(max_offset_px)
    pad = (int(max_offset_px[0]), int(max_offset_px[1]))
    ...
    if remove_outliers:
        diff = outlier_filter(diff, f=2, max_dz=max_dz)
    ...
    # Nuth and Kaab (2011)
        ...
        if fit_param is None:
            ...
            # print(fit_param)
            dx = np.array(fit_param[0] * np.sin(np.deg2rad(fit_param[1])))
            dy = np.array(fit_param[0] * np.cos(np.deg2rad(fit_param[1])))
            med_slope = malib.fast_median(slope)
            nuth_dz = np.array(fit_param[2] * np.tan(np.deg2rad(med_slope)))
            print('Median dz: %0.2f\nNuth dz: %0.2f' % (dz, nuth_dz.mean()))
            # dz = nuth_dz
    elif mode == "nuth_block":
        # to do: check image size
        dx, dy, dz, fig = coreglib.compute_offset_nuth_block(diff, slope, aspect, blk_sz=blk_sz, min_count=2000,
                                                             remove_outliers=True, plot=True)
        # xyz_shift_str_iter = "dx=%+0.2fm, dy=%+0.2fm, dz=%+0.2fm" % offsets[i,0], offsets[i,1], offsets[i,2],
        # print("Block %i: %s " %i, xyz_shift_str_iter)

        # fig.savefig('block'+str(i)+'.png', dpi=300)

        # fit_param, fig = coreglib.compute_offset_nuth_blk(diff, slope, aspect, blk_sz=300, plot=plot)
        ...
        # iolib.writeGTiff(src_dem + dz, dst_fn, src_dem_ds)
        sys.exit()
...
def getparser():
    ...
    parser.add_argument('-mode', type=str, default='nuth', choices=['ncc', 'sad', 'nuth', 'nuth_block', 'none'],
    ...
    parser.add_argument('-blk_sz', type=int, nargs=2, default=(300,300),
                        help='Size of individual block for Block-wise nuth co-registration')
    ...
    parser.add_argument('-tol', type=float, default=None,
                        help='When iterative translation magnitude is below this tolerance (meters) (default: 1/50 of 1'
                            'pixel), break and write out corrected DEM')
    parser.add_argument('-max_offset', type=float, default=100,
                        help='Maximum expected horizontal offset in meters, used to set search range for ncc and sad '
                             'modes')
    parser.add_argument('-max_dz', type=float, default=100,
                        help='Maximum expected vertical offset in meters, used to filter outliers')
    res_choices = ['min', 'max', 'mean', 'first', 'last', 'common_scale_factor']
    parser.add_argument('-res', type=str, default='last', choices=res_choices,
                        help='Warp intputs to this resolution')
    parser.add_argument('-slope_lim', type=float, nargs=2, default=(0.01, 50),
...
def main(args=None):
    ...
    src_dem_ds = None
    # Resample to user-specified resolutiondh_blk
    ref_dem_ds, src_dem_ds_align = warplib.memwarp_multi([ref_dem_ds, src_dem_ds_align],
                                                         extent='intersection', res=res, t_srs=local_srs,
                                                         r='cubic')
    if mode == 'nuth_block':
        bma = iolib.ds_getma(src_dem_ds_align)
        dst_nl, dst_ns = blk_sz[0] * round(bma.shape[0] / blk_sz[0]), blk_sz[1] * round(bma.shape[1] / blk_sz[1])
        src_dem_ds_align = warplib.warp(src_dem_ds_align, size=[dst_nl, dst_ns])
        local_srs = geolib.get_ds_srs(src_dem_ds_align)
        ref_dem_ds, src_dem_ds_align = warplib.memwarp_multi([ref_dem_ds, src_dem_ds_align],
                                                             extent='intersection', res=res, t_srs=local_srs,
                                                             r='cubic')
    ...
    print("\nReference DEM res: %0.2f m  %0.2f m" % (ref_dem_res[0], ref_dem_res[1]))
    print("Source DEM res: %0.2f m  %0.2f m" % (src_dem_res[0], src_dem_res[1]))
    print("Resolution for coreg: %s (%0.2f m  %0.2f m)\n" % (args.res, res[0], res[1]))

    if tol is None:
        tol = 0.01 * np.mean(res)
        if tol < 0.2:
            tol = 0.2
    ...
    n = 1
    # Cumulative offsets
    dx_total = float(np.array(0))
    dy_total = float(np.array(0))
    dz_total = float(np.array(0))
    ...
    while True:
        ...
        dx, dy, dz, static_mask, fig = compute_offset(ref_dem_ds, src_dem_ds_align, src_dem_fn, mode, max_offset,
                                                      mask_list=mask_list, max_dz=max_dz, blk_sz=blk_sz, slope_lim=slope_lim,
                                                      plot=True)
        xyz_shift_str_iter = "dx=%+0.2fm, dy=%+0.2fm, dz=%+0.2fm" % (dx.mean(), dy.mean(), dz.mean())
        ...
        xyz_shift_str_cum = "dx=%+0.2fm, dy=%+0.2fm, dz=%+0.2fm" % (dx_total.mean(), dy_total.mean(), dz_total.mean())
        print("Cumulative offset: %s" % xyz_shift_str_cum)
        # String to append to output filenames
        xyz_shift_str_cum_fn = '_%s_x%+0.2f_y%+0.2f_z%+0.2f' % (mode, dx_total.mean(), dy_total.mean(), dz_total.mean())
        ...
        dm = np.sqrt(dx.mean() ** 2 + dy.mean() ** 2 + dz.mean() ** 2)
        dm_total = np.sqrt(dx_total.mean() ** 2 + dy_total.mean() ** 2 + dz_total.mean() ** 2)
        ...
        if n > max_iter or dm < tol:
            ...
            if True:
                ...
                diff_align_filt[np.where(mask_glac == True)] = diff_align_glac_outlier[mask_glac == True]
            ...
            if tiltcorr and not tiltcorr_done:
                ...
                stripes, fig = coreglib.fft_destripe(diff_align_filt, blocks=None, mask=mask_glac, filt_sz=5,
                                                     std_th=0.05, percentile_th=97.5, plot=True)
    ...
    src_dem_ds = gdal.Open(src_dem_fn)
    src_dem_ds_align = iolib.mem_drv.CreateCopy('', src_dem_ds_align, 0)
    ...
    if True:
        ...
        align_stats['shift'] = {'dx': dx_total.mean(), 'dy': dy_total.mean(), 'dz': dz_total.mean(), 'dm': dm_total.mean()}
    ...
    if True:
        ...
        shp_fn = os.path.join(datadir, 'randoph/rgi60_asia_merge_single_line.shp')
        ...
        dz_clim = malib.calcperc_sym(diff_orig_compressed, (2, 98))
        ...
        suptitle = '%s\nx: %+0.2fm, y: %+0.2fm, z: %+0.2fm' % (
            os.path.split(outprefix)[-1], dx_total.mean(), dy_total.mean(), dz_total.mean())
        f.suptitle(suptitle)
        f.tight_layout()
